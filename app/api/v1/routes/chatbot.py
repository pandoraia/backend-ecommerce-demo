...
async def handle_user_query(query: str):
    ...
    complementary_products = recommended_products.get(
        "complementary_products", [])

    # Convertir productos complementarios a un formato adecuado (string o lista)
    complementary_products_str = ", ".join(
        [f"{p['name']} ({p['description']})" for p in complementary_products[:2]])  # Limitar a 2 productos
